...
public class LogSave {
  ...
  LogSave() throws IOException {
    ...
    f1.write("Share:\t\t\t" + WorldParams.eShare + "\r\n");
    ...
    f1.write(
        "Period\tAgents\t"
            // actions stats
            + "Rest\tEat\tMove\tTurnLeft\tTurnRight\tDivide\tFight\tShare\t"
            // input stats
            + "inLevel\tinGrNear\tinGrLeft\tinGrForw\tinGrRight\tinAgNear\tinAgLeft\tinAgForw\tinAgRight\tinCellKin\tinEnergy\tinNegEnergy\tinAgKin\tinAgEnrg\tinNegAgEnrg\t"
            // output stats
            + "actRest\tactEat\tactMove\tactTurnLeft\tactTurnRight\tactDivide\tactFight\tactShare\t"
            + "Avg.Ener\tAvgDivedPeriod  \r\n");
  }
  ...
  public void saveLog(Vector v, int totalAgents, int t) throws IOException {
    ...
    for (int z = 0; z < World2D.population; z++) {
    ...
    float aver = 0;
    if (World2D.population != 0)
      aver = averageEnergy / World2D.population;
    averageDividePeriod = averageDividePeriod / agn;
    int per = WorldParams.period;
    if (t == 0) {
      per = 1;
      totalAgents = World2D.population;
    }
    f1.write(t + "\t" + totalAgents + "\t");
    for (int i = 0; i < Genome.nAct; i++)
      f1.write((int) (World2D.actions[i] / per) + "\t"); // f1.write(act[i] + "\t");
    for (int i = 0; i < Genome.nIn; i++)
      f1.write(in[i] + "\t");
    for (int i = 0; i < Genome.nAct; i++)
      f1.write(out[i] + "\t");
